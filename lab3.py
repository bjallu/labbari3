...
# ## Bayes cljassifier functions to implement
# 
...
def mlParams(X, labels, W=None):
    ...
    for jdx,klass in enumerate(classes):
        ...
        n = len(xlc)
        tmp = np.zeros(Ndims)
        for j,x in enumerate(xlc):
            tmp += x*tmp_w[j]
        mu[jdx]=tmp/sum(tmp_w)
    ...
    for jdx,klass in enumerate(classes):
        ...
        tmp_w = W[idx]
        xlc = X[idx,:]
        sigmavalue = sum(np.square(xlc - mu[jdx])*tmp_w)/sum(tmp_w)
        for i in range(len(sigmavalue)):
            sigma[jdx,i,i] += sigmavalue[i]
    # ==========================
...
def classifyBayes(X, prior, mu, sigma):
    ...
    for k in range(Nclasses):
        for i in range(Npts):
            tmp = -0.5*np.log(np.linalg.det(sigma[k])) -0.5*(np.matmul(np.matmul((X[i]-mu[k]),np.linalg.pinv(sigma[k])), (X[i]-mu[k]).T))+np.log(prior[k])
            logProb[k,i]=tmp
    # ==========================
...
W=np.asarray([1/len(X) for x in range(len(X))])
W.shape = (len(X),1)
